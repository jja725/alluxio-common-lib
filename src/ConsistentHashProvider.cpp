...
std::vector<uint8_t> uuid_string_to_bytes(const std::string& uuid_str) {
    ...
    return std::vector<uint8_t>(uuid, uuid + sizeof(uuid_t));
}
...
WorkerEntity WorkerEntity::from_worker_info(const std::string& worker_info) {
    try {
        ...
        WorkerIdentity worker_identity(
            ...
            uuid_string_to_bytes(identity_info.get("identifier", "").asString()));
...
WorkerEntity WorkerEntity::from_host_and_port(const std::string& worker_host, int worker_http_port) {
    uuid_t uuid;
    // uuid_generate_md5(uuid, uuid_nil, worker_host.c_str(), worker_host.size());
    std::vector<uint8_t> uuid_bytes(uuid, uuid + sizeof(uuid_t));

    WorkerIdentity worker_identity(DEFAULT_WORKER_IDENTIFIER_VERSION, uuid_bytes);
    WorkerNetAddress worker_net_address(
        worker_host,
