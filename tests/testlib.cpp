...
void validate_hash_ring(const std::map<int32_t, WorkerIdentity> &current_ring,
...
TEST_CASE_METHOD(ConsistentHashProviderFixture, "Test hash ring",
                 "[ConsistentHashProvider]") {
  alluxio::ConsistentHashProvider hash_provider(config);
  ...
  for (const auto &worker_data : workers_data) {
    auto identifier = WorkerIdentity::get_bytes_from_hex_string(
        worker_data["identifier"].asString());
    WorkerIdentity worker_identity(worker_data["version"].asInt(), identifier);
  ...
  for (const auto &member : file_workers_data.getMemberNames()) {
    ...
    std::vector<WorkerIdentity> current_worker_identities =
        hash_provider._get_multiple_worker_identities(
            ufs_url, DEFAULT_NUM_VIRTUAL_NODES);

    std::set<std::pair<int, std::vector<char>>> original_set;
    for (const auto &worker : workers) {
      auto identifier = WorkerIdentity::get_bytes_from_hex_string(
          worker["identifier"].asString());
      original_set.insert(
          std::make_pair(worker["version"].asInt(), identifier));
    }
...
TEST_CASE("WorkerEntity from info", "[WorkerEntity]") {
  ...
  std::string workerInfoStr = writer.write(workerInfoDict);
  ...
  REQUIRE(workerEntity.worker_identity.version ==
          fieldValues["version"].asInt());
  auto identifier = WorkerIdentity::get_bytes_from_hex_string(
      fieldValues["identifier"].asString());
  REQUIRE(workerEntity.worker_identity.identifier == identifier);

  // Validate WorkerNetAddress fields
  REQUIRE(workerEntity.worker_net_address.host ==
          fieldValues["Host"].asString());
  REQUIRE(workerEntity.worker_net_address.container_host ==
          fieldValues["ContainerHost"].asString());
  REQUIRE(workerEntity.worker_net_address.rpc_port ==
          fieldValues["RpcPort"].asInt());
  REQUIRE(workerEntity.worker_net_address.data_port ==
          fieldValues["DataPort"].asInt());
  REQUIRE(workerEntity.worker_net_address.secure_rpc_port ==
          fieldValues["SecureRpcPort"].asInt());
  REQUIRE(workerEntity.worker_net_address.netty_data_port ==
          fieldValues["NettyDataPort"].asInt());
  REQUIRE(workerEntity.worker_net_address.web_port ==
          fieldValues["WebPort"].asInt());
  REQUIRE(workerEntity.worker_net_address.domain_socket_path ==
          fieldValues["DomainSocketPath"].asString());
  REQUIRE(workerEntity.worker_net_address.http_server_port ==
          fieldValues["HttpServerPort"].asInt());
}

TEST_CASE("Test hash value of WorkerEntity", "[ConsistentHashProvider]") {
  // Create a WorkerIdentity
  const std::string identifier_hex = "4f23e7a1a230318a912dbdf8f78e0aed";
  auto identifier = WorkerIdentity::get_bytes_from_hex_string(identifier_hex);
  ...
  std::unordered_set<int32_t> expected_hashes = {
      -2035703500, -2020503163, -1945615066, -1905409545, 1420996528};

  // Loop over node indices from 0 to 4
  for (int node_index = 0; node_index < DEFAULT_NUM_VIRTUAL_NODES;
       ++node_index) {
    // Calculate the hash value
    int32_t calculated_hash =
        hash_provider._hash_worker_identity(worker_identity, node_index);
...
TEST_CASE("Test AlluxioClient", "[AlluxioClient]") {
  ...
  std::vector<std::string> testFiles = {
      "s3://file1.txt", "s3://file2.txt", "s3://file3.txt", "s3://file4.txt",
      "s3://file5.txt", "s3://file6.txt", "s3://file7.txt", "s3://file8.txt",
      "s3://file9.txt", "s3://file10.txt"};
  ...
  for (const auto &file : testFiles) {
    ...
    std::vector<ReadLocation> workerAddresses =
        client.getWorkerAddress(file, 0, 1024);
